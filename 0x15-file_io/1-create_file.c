 ...
 * @filename: given name to new file
...
int create_file(const char *filename, char *text_content)
{
	...
	/* create with permissions if file doesn't exist if else "truncate" */
	fd = open(filename, O_CREAT | O_WRONLY | O_TRUNC, 0600);
	if (fd == -1)
		return (-1);

	/* if there is nothing to write, just return newly created file */
	if (!text_content)
	{
		close(fd);
		return (1);
